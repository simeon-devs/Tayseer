...
import type { CaseListItem, RiskSummary } from '../../../lib/types';
...
export default function StaffCaseQueue() {
  ...
  const filteredCases = allCases.filter((c) => {
    ...
    const matchesRisk = riskFilter ? (riskMap.get(c.emirates_id) ?? 'LOW') === riskFilter : true;
        ...
        <main className="max-w-7xl mx-auto px-4 py-8 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between mb-8">
            <div>
              ...
              <p className="text-sm font-medium text-gray-500 mt-1">{t('tagline')}</p>
            </div>
          ...
          {/* Risk Intelligence KPI Cards */}
          <div className="mb-8">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-bold text-gray-700">{t('riskBannerTitle')}</span>
              <div className="flex items-center gap-3 text-xs text-gray-400">
                {riskFilter && (
                  <button type="button" onClick={() => setRiskFilter(null)} className="text-primary underline font-semibold">
                    {t('clearRiskFilter')}
                  </button>
                )}
                {lastAnalysedLabel && <span>{t('lastAnalysed')}: {lastAnalysedLabel}</span>}
                <button
                  type="button"
            ...
            {riskLoading ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                <button type="button" onClick={() => handleRiskZoneClick('HIGH')}
                  className={`bg-white rounded-2xl border-s-4 border-red-600 shadow-sm border border-gray-200/50 p-6 relative overflow-hidden flex items-center justify-between hover:shadow-md transition-all text-left ${riskFilter === 'HIGH' ? 'ring-2 ring-red-300' : ''}`}>
                  <div className="space-y-1">
                    <span className="text-xs font-bold text-red-600 tracking-wider uppercase">{t('highRiskZone')}</span>
                    <p className="text-4xl font-black text-gray-900 leading-tight">{riskSummary?.high_risk_count ?? 0}</p>
                ...
                <button type="button" onClick={() => handleRiskZoneClick('MEDIUM')}
                  className={`bg-white rounded-2xl border-s-4 border-amber-500 shadow-sm border border-gray-200/50 p-6 relative overflow-hidden flex items-center justify-between hover:shadow-md transition-all text-left ${riskFilter === 'MEDIUM' ? 'ring-2 ring-amber-300' : ''}`}>
                  <div className="space-y-1">
                    <span className="text-xs font-bold text-amber-600 tracking-wider uppercase">{t('mediumRiskZone')}</span>
                    <p className="text-4xl font-black text-gray-900 leading-tight">{riskSummary?.medium_risk_count ?? 0}</p>
                ...
                <button type="button" onClick={() => handleRiskZoneClick('LOW')}
                  className={`bg-white rounded-2xl border-s-4 border-emerald-500 shadow-sm border border-gray-200/50 p-6 relative overflow-hidden flex items-center justify-between hover:shadow-md transition-all text-left ${riskFilter === 'LOW' ? 'ring-2 ring-emerald-300' : ''}`}>
                  <div className="space-y-1">
                    <span className="text-xs font-bold text-emerald-600 tracking-wider uppercase">{t('safeZone')}</span>
                    <p className="text-4xl font-black text-gray-900 leading-tight">{riskSummary?.low_risk_count ?? 0}</p>
          ...
          <div className="bg-white rounded-2xl border border-gray-200/60 shadow-sm overflow-hidden mb-12">
            ...
            <div className="p-5 border-b border-gray-200/60 flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-gray-50/20">
              ...
              <div className="flex items-center gap-3 self-end md:self-center text-xs text-gray-500 font-medium">
                {lastUpdated && <span>{t('lastAnalyzed')}: {formatTime(lastUpdated)}</span>}
                <button
                  onClick={fetchCases}
                  disabled={loading}
                ...
                >
                  <svg className={`w-3.5 h-3.5 ${loading ? 'animate-spin text-primary' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.2}>
            ...
            <div className="px-6 py-5 border-b border-gray-100/70 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div>
                <h2 className="text-lg font-bold text-gray-900 font-arabic-safe">{t('allCases')}</h2>
                <p className="text-xs font-semibold text-gray-400 mt-0.5">{t('staffNote')}</p>
              </div>
            ...
            {!loading && !error && paginatedCases.length === 0 && (
              <div className="py-20 text-center text-gray-400 font-semibold text-sm">
                {search || filter || riskFilter ? t('noCasesMatch') : t('noCasesFound')}
              </div>
            )}
            ...
            {!loading && !error && paginatedCases.length > 0 && (
              <>
                    ...
                    <tbody className="divide-y divide-gray-100/70">
                      {paginatedCases.map((c) => (
                        <tr key={c.id} className="hover:bg-slate-50/40 transition-colors">
                          ...
                          <td className="px-6 py-4 font-mono text-xs text-gray-600 font-medium">{c.emirates_id}</td>
                          ...
                          <td className="px-6 py-4 text-gray-600 max-w-sm truncate text-xs font-medium">
                            {c.decision_summary ?? <span className="text-gray-400 italic font-normal">{t('noDecision')}</span>}
                          </td>
                          <td className="px-6 py-4">
                            <Link href={`/staff/cases/${c.id}`} className="inline-flex items-center gap-1 text-gold font-bold hover:text-gold-dark text-sm group">
                  ...
                  {paginatedCases.map((c) => (
                    <div key={c.id} className="p-5 flex flex-col gap-3.5 hover:bg-slate-50/30 transition-colors">
                      <div className="flex items-center justify-between">
                        <span className="font-mono text-xs text-gray-400 font-semibold">{c.id.split('-')[0].toUpperCase()}</span>
                        <StatusBadge status={c.status} />
                      </div>
                      <div>
                        <div className="flex items-center gap-1.5">
                          <RiskDot level={riskMap.get(c.emirates_id)} />
                          <p className="font-bold text-gray-900 text-sm">{lang === 'ar' ? c.citizen_name_ar : c.citizen_name_en}</p>
                        </div>
                        <p className="text-xs text-gray-500 font-medium mt-1">{c.emirates_id}</p>
                      </div>
                      <div className="flex items-center justify-between border-t border-gray-100 pt-3">
                        ...
                        <Link href={`/staff/cases/${c.id}`} className="inline-flex items-center gap-1 bg-gold text-white font-bold text-xs py-2 px-4 rounded-xl hover:bg-gold-dark shadow-sm active:scale-95">
                          <span>{t('viewDetails')}</span>
                        </Link>
                      </div>
                    </div>
                ...
                {/* Pagination */}
                <div className="px-6 py-4 border-t border-gray-200/60 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-gray-50/20 text-sm">
                  ...
                  <div className="flex items-center justify-center gap-1.5">
                    <button onClick={() => setCurrentPage(currentPage - 1)} disabled={currentPage === 1}
                      className="p-2 border border-gray-300 rounded-xl hover:bg-gray-50 text-gray-500 disabled:opacity-40 shadow-sm bg-white transition-colors flex items-center justify-center min-w-[36px] h-9">
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                      <button key={page} onClick={() => setCurrentPage(page)}
                        className={`w-9 h-9 text-xs font-bold rounded-xl shadow-sm transition-all ${currentPage === page ? 'bg-primary text-white' : 'bg-white border border-gray-300 text-gray-600 hover:bg-gray-50'}`}>
                        {page}
                      </button>
                    ))}
