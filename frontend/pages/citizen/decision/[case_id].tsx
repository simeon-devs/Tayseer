...
function DecisionCard({ detail, caseId }: { detail: CaseDetailResponse; caseId: string }) {
  ...
  const isApproved = decision && !decision.escalate_flag;
  const isEscalated = decision && decision.escalate_flag;

  return (
    <div className="space-y-5">
      <div
        className={`rounded-2xl p-6 text-white ${
          isApproved ? 'bg-approved' : 'bg-escalated'
        }`}
      >
        <div className="flex items-start gap-4">
          <StatusIcon approved={!!isApproved} />
          <div>
            <h1 className="text-2xl font-bold">
              {isApproved ? t('approvedTitle') : t('escalatedTitle')}
            </h1>
            <p className="opacity-90 mt-1 text-sm">
              {isApproved ? t('approvedSubtitle') : t('escalatedSubtitle')}
            </p>
          </div>
        </div>
      </div>
      ...
      {isApproved && decision && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <h2 className="font-bold text-gray-900 text-lg mb-4">{lang === 'ar' ? 'تفاصيل القرار' : 'Decision Details'}</h2>
      ...
      {decision && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
          <h2 className="font-bold text-gray-900 mb-3">
            {isEscalated ? t('escalationReason') : t('rationale')}
          </h2>
          {isEscalated ? (
            <>
